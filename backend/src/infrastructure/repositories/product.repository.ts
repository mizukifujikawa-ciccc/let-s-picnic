...
// get product by name
const getProductByName = async (productName: string): Promise<Product | null> => {
  const client = createClient()
  try {
    await client.connect()
    const result = await client.query(
...
const getProductsByCategoryId = async (categoryId: number): Promise<Product[]> => {
  const client = createClient()
  try {
    await client.connect()
    const result = await client.query(
...
// create product
const createProduct = async (newProduct: Omit<Product, 'id'| 'createdAt' | 'updatedAt'>): Promise<Product | null> => {
  ...
  try {
    ...
    return cat ? mapRowToProduct({ ...inserted, ...cat, category_id: inserted.category_id }) : null
  } catch (err) {
    console.error(err)
    throw err
  } finally {
...
// edit product by id
const editProduct = async (id: number, updatedProduct: Partial<Product>): Promise<Product | null> => {
  ...
  try {
    ...
    const updateData = {
      ...
      discountPercentage: updatedProduct.discountPercentage ?? foundProduct.discountPercentage,
    ...
    const cat = joined.rows[0]
    return cat ? mapRowToProduct({ ...updated, ...cat, category_id: updated.category_id }) : null
  } catch (err) {
    console.error(err)
    throw err
  } finally {
